...
훈노트는 1인 개발자, 프리랜서, 창작자, 개인 사업자가 업무·아이디어·할 일·프로젝트 메모를 정리하는 로그인형 클라우드 노트입니다. 복잡한 협업 기능보다 혼자 빠르게 기록하고 다시 찾는 경험을 우선합니다.
...
- `js/ui.js`: 메모·상세·필터 UI 렌더링
- `js/app.js`: 앱 상태, 이벤트, 동기화, 카테고리 관리
- `js/account.js`: 계정 삭제
...
v4.5.3은 데이터베이스 구조를 변경하지 않습니다. 메모 카테고리 `할 일`은 UI에서 제거되고, 이전 데이터에 남은 값은 클라이언트에서 `미분류`로 호환합니다. v4.5의 `05_create_memo_categories.sql`을 이미 실행했다면 추가 SQL은 없습니다. v4.5.3에서는 클라우드 조회의 사용자 조건과 백업 복원 검증을 보강했습니다.
